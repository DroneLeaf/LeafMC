...
void Joystick::_requestConfirmActionStart()
{
    QMetaObject::invokeMethod(qgcApp(), []() {
        QQmlApplicationEngine* engine = qgcApp()->qmlAppEngine();
        if (engine) {
            ...
            if (!roots.isEmpty()) {
                QObject* rootObj = roots.first();
                if (rootObj) {
                    QMetaObject::invokeMethod(rootObj, "confirmActionStarted", Qt::QueuedConnection);
                }
            }
        }
    }, Qt::QueuedConnection);
...
void Joystick::_requestConfirmActionStop()
{
    QMetaObject::invokeMethod(qgcApp(), []() {
        QQmlApplicationEngine* engine = qgcApp()->qmlAppEngine();
        if (engine) {
            ...
            if (!roots.isEmpty()) {
                QObject* rootObj = roots.first();
                if (rootObj) {
                    QMetaObject::invokeMethod(rootObj, "confirmActionStopped", Qt::QueuedConnection);
                }
            }
        }
    }, Qt::QueuedConnection);
...
void Joystick::_executeButtonAction(const QString& action, bool buttonDown)
{
    ...
    // Confirmation popup action (hold to confirm like spacebar)
    else if(action == _buttonActionConfirmAction) {
        if (buttonDown) {
            _requestConfirmActionStart();
        } else {
            _requestConfirmActionStop();
        }
    }
    else {
